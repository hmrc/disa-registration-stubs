...
object Postcode {
  ...
  private[addresslookup] val oPattern =
    Pattern.compile("^GIR|[A-Z]{1,2}[0-9][0-9A-Z]?$")
  private[addresslookup] val iPattern = Pattern.compile("^[0-9][A-Z]{2}$")

  /** Performs normalisation and then checks the syntax, returning None if the string cannot represent a well-formed
    * postcode.
    */
